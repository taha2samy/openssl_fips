...
### Production-Grade FIPS 140-3 Compliant OpenSSL Container Image
...
[![OpenSSL Version](https://img.shields.io/badge/OpenSSL-3.5.5-0078D4?logo=openssl&logoColor=white&style=for-the-badge)](https://www.openssl.org/)
...
[![Build Status](https://img.shields.io/github/actions/workflow/status/taha2samy/wolfi-openssl-fips/build.yml?style=for-the-badge&logo=githubactions&logoColor=white)](https://github.com/taha2samy/openssl_fips/actions)
...
**Wolfi OpenSSL FIPS Images** delivers **production-hardened**, **FIPS 140-3 compliant** OpenSSL 3.5.5 container images engineered for mission-critical environments where security, compliance, and auditability are non-negotiable.
...
| **Image Size** | 40-100MB+ | ✅ **distroless: 0B | standard: 0B** |
...
docker run --rm -it ghcr.io/taha2samy/wolfi-openssl-fips:latest sh

# Run specific command
docker run --rm ghcr.io/taha2samy/wolfi-openssl-fips:latest version
...
# ==================================
FROM ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5 AS fips-openssl
...
services:
  openssl-fips:
    image: ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5
...
```yaml
.fips-template:
  image: ghcr.io/taha2samy/wolfi-openssl-fips:distroless
  before_script:
...
```bash
docker run --rm ghcr.io/taha2samy/wolfi-openssl-fips:latest list -providers
```
...
```bash
docker run --rm ghcr.io/taha2samy/wolfi-openssl-fips:latest \
...
[
  {
    "critical": {
      "identity": {
        "docker-reference": "ghcr.io/taha2samy/wolfi-openssl-fips"
      },
      "image": {
...
cosign verify-attestation \
  ...
  --certificate-identity-regexp "^https://github.com/taha2samy/openssl_fips/.*" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5 | jq -r '.payload' | base64 -d | jq
```
...
```json
{
  "predicate": {
    ...
    "builder": {
      "id": "https://github.com/taha2samy/wolfi-openssl-fips/.github/workflows/build.yml@refs/heads/main"
    },
    "invocation": {
      "configSource": {
        "uri": "git+https://github.com/taha2samy/wolfi-openssl-fips@refs/heads/main",
        "digest": {
          "sha1": "..."
        }
      }
...
```bash
# Using Trivy
trivy image ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5

# Using Grype
grype ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5

# Using Docker Scout
docker scout cves ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5
```
...
# Usage
verify_fips_openssl("ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5")
```
...
| RSA 2048 Sign | ops/s | 7,045.5 | 7,132.1 | 6,909.7 | 7,024.0 |
| RSA 2048 Verify | ops/s | 124,874.5 | 123,103.5 | 118,850.5 | 123,093.5 |
| Ed25519 Sign | ops/s | 65,917.0 | 45,779.0 | 45,737.0 | 45,215.5 |
| Ed25519 Verify | ops/s | 19,530.5 | 17,196.0 | 18,409.0 | 17,041.5 |
| AES-256-GCM | kB/s | 21,374,836.7 | 8,784,584.7 | 21,191,761.9 | 23,696,228.4 |
| SHA-256 | kB/s | 3,621,126.1 | 3,603,890.2 | 3,512,533.0 | 3,565,543.4 |
| SHA-512 | kB/s | 1,425,481.7 | 1,422,852.1 | 1,421,942.8 | 1,424,556.0 |
...
```bash
# Standard image
docker images ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5 --format "{{.Size}}"
# Output: ~0B

# Distroless image
docker images ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5-distroless --format "{{.Size}}"
# Output: ~0B

# Layer breakdown
docker history ghcr.io/taha2samy/wolfi-openssl-fips:3.5.5 --no-trunc
```
...
for i in {1..10}; do
  /usr/bin/time -f "%E" docker run --rm \
    ghcr.io/taha2samy/wolfi-openssl-fips:distroless \
...
docker run --rm ghcr.io/taha2samy/wolfi-openssl-fips:latest dgst -md5 /etc/passwd
...
docker run --rm ghcr.io/taha2samy/wolfi-openssl-fips:latest help
...
[![GitHub Stars](https://img.shields.io/github/stars/taha2samy/openssl_fips)](https://github.com/taha2samy/openssl_fips/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/taha2samy/openssl_fips)](https://github.com/taha2samy/openssl_fips/network/members)
[![GitHub Issues](https://img.shields.io/github/issues/taha2samy/openssl_fips)](https://github.com/taha2samy/openssl_fips/issues)
[![GitHub PRs](https://img.shields.io/github/issues-pr/taha2samy/openssl_fips)](https://github.com/taha2samy/openssl_fips/pulls)
