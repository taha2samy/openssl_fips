{% set cis_data = distroless_compliance['docker_cis'] %}
...
# :fontawesome-brands-docker: CIS Docker Benchmark (Distroless)
...
-   :material-shield-check: **Automated Score**
    ...
    *Verified on {{ total_auto }} Automated Checks*
...
-   :material-alert-circle-outline: **Hardening Failures**
    ---
    {% if failed_auto == 0 -%}
    ...
    *Requires Image Update*

</div>

{% if failed_auto > 0 %}
!!! failure "Compliance Gaps in Production Image"
    The Distroless image failed some automated CIS checks. These should be remediated to minimize the attack surface in production environments.
{% elif ns.manual | length > 0 %}
!!! success "Automated Hardening Verified"
...
*   :material-check-circle:{ .md-typeset__success } **Passed:** Policy is enforced within the distroless layers.
*   :material-close-circle:{ .md-typeset__error } **Failed:** Violation detected (e.g., setuid permissions).
*   :material-clipboard-alert:{ style="color: #2979ff" } **Manual:** Host-dependent check (e.g., Docker Daemon logging, Content Trust).

---
[:material-arrow-up: Back to Top](#cis-docker-benchmark-distroless)